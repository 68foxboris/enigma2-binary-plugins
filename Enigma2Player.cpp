...
bool CEnigma2Player::OpenFile(const CFileItem& file, const CPlayerOptions &options)
{
  try
  {
    ...
    if (file.OpenForWrite(m_filenameMeta, true))
    {
        ...
        else if (ret != (ssize_t)jsonStr.length())
        {
...
void CEnigma2Player::Process()
{
  ...
  CLog::Log(LOGINFO, "Enigma2Player: terminado a {} segundos", m_time);
//  IPlayerCallback *cb = &m_callback;
...
/*  CJobManager::GetInstance().Submit([=]() {
    ...
    CLog::Log(LOGINFO, "Enigma2Player: bookmark a {} segundos", duration.count() / 1000);
  }
...
bool CEnigma2Player::CanSeek() const
{
  return false;
}
...
float CEnigma2Player::GetPercentage()
{
  ...
  if (iTotalTime != 0)
  {
    CLog::Log(LOGDEBUG, "Percentage is {}", (iTime * 100 / (float)iTotalTime));
...
bool CEnigma2Player::Initialize(TiXmlElement* pConfig)
{
  ...
  {
    CLog::Log(LOGINFO, "Enigma2Player Filename: {}", m_filename);
  }
  else
  {
    std::string xml;
    xml<<*pConfig;
    CLog::Log(LOGERROR, "Enigma2Player Error: filename element missing from: {}", xml);
    return false;
  }
  ...
  {
    CLog::Log(LOGINFO, "Enigma2Player FilenameMeta: {}", m_filenameMeta);
  }
  ...
  if (XMLUtils::GetString(pConfig, "warpcursor", warpCursor) && !warpCursor.empty())
  {
    ...
    {
      warpCursor = "none";
      CLog::Log(LOGWARNING, "Enigma2Player: invalid value for warpcursor: {}", warpCursor);
    }
  }
  ...
  while (pReplacer)
  {
    if (pReplacer->FirstChild())
    {
      ...
      if (!strPat.empty() && !strRep.empty())
      {
        ...
        CLog::Log(LOGDEBUG,"    Match:[{}] Pattern:[{}] Replacement:[{}]", strMatch, strPat, strRep);
...
void CEnigma2Player::handleAppLinuxOutput(const std::string& output)
{
  ...
  CLog::Log(LOGINFO, "{}: position: {}, duration: {}", __FUNCTION__, m_time, m_totalTime);
}
